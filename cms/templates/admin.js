...
/**
 * Returns the admin page content to be inserted into the admin base template.
 * @param {Object} params
 * @param {string} params.currentLanguage - The current language
 * @param {string} params.messageType - Optional message type to display
 * @param {Array} params.entries - Array of entries from the database
 * @returns {string} The HTML content for the admin page.
 */
export const adminPageContent = ({ currentLanguage, entries = [] }) => `
                ...
                ${entries.map(entry => `
                  <tr>
                    ...
                    <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-900 w-20">
                      <span class="inline-flex px-1 py-1 text-xs font-semibold rounded ${entry.type === 'blog' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}">${entry.type}</span>
                    </td>
                    <td class="px-3 py-3 text-sm text-gray-900 max-w-32">
                      <div class="truncate font-mono text-xs">${entry.slug || entry.id}</div>
                    </td>
                    ...
                    <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-900 w-16">
                      <span class="text-xs font-semibold px-1 py-1 rounded bg-purple-100 text-purple-800">${entry.language || 'en'}</span>
                    </td>
                    <td class="px-3 py-3 whitespace-nowrap text-xs text-gray-500 w-24">${new Date(entry.created_at).toLocaleDateString('en-US', {month: 'short', day: 'numeric'})}</td>
                    <td class="px-3 py-3 whitespace-nowrap text-xs text-gray-500 w-24">${new Date(entry.modified_at).toLocaleDateString('en-US', {month: 'short', day: 'numeric'})}</td>
                    <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-900 w-16">${entry.created_by}</td>
                    <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-900 w-20">
                      <span class="inline-flex px-1 py-1 text-xs font-semibold rounded ${entry.draft ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}">${entry.draft ? 'Draft' : 'Pub'}</span>
                    </td>
                    <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-900 w-16">
