 ...
 */
export const adminBaseTemplate = ({ content, ref, currentLanguage, loggedIn, messageType = '' }) => `
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    ...
    <div class="min-h-screen bg-gradient-to-b from-white to-gray-50">
      ...
      <header class="bg-white/90 backdrop-blur-sm fixed top-0 left-0 right-0 z-40 transition-all duration-300">
        <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
          ...
          ${ loggedIn ? `
            <a href="/admin" class="text-sm font-semibold leading-6 text-gray-900 hover:text-green-600 transition-colors duration-200">${locale[currentLanguage].nav.dashboard}</a>
            <!--
            <a href="/admin/settings" class="text-sm font-semibold leading-6 text-gray-900 hover:text-green-600 transition-colors duration-200">${locale[currentLanguage].nav.settings}</a>
            <a href="/admin/users" class="text-sm font-semibold leading-6 text-gray-900 hover:text-green-600 transition-colors duration-200">${locale[currentLanguage].nav.users}</a>
            -->
            <a href="/admin/logout" class="text-sm font-semibold leading-6 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors duration-200">${locale[currentLanguage].nav.logout}</a>
          ` : `` }
            <div class="relative group nav-dropdown-parent">
              ...
              <div id="lang-dropdown" class="hidden absolute right-0 top-full pt-3 z-50 rounded-xl bg-white p-4 shadow-lg ring-1 ring-gray-900/5">
                <a href="/admin/set-lang?lang=en&ref=${ref}" class="block rounded-lg px-3 py-2 text-sm leading-6 text-gray-900 hover:bg-gray-50 hover:text-green-600 transition-colors duration-200">EN</a>
                <a href="/admin/set-lang?lang=de&ref=${ref}" class="block rounded-lg px-3 py-2 text-sm leading-6 text-gray-900 hover:bg-gray-50 hover:text-green-600 transition-colors duration-200">DE</a>
              </div>
            </div>
          </div>
        </nav>
