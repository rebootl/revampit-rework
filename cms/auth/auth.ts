...
export function isLoggedIn(req: Request, res: Response, next: () => void): void {
  ...
  try {
    ...
    if (!session) {
      res.status(401).send('Invalid session. Please clear your cookies and log in again.');
      return;
    }
...
// Login endpoint
app.post('/admin/login', async (req: Request<{}, {}, LoginRequest>, res: Response): Promise<void> => {
  const { username, password } = req.body;

  // Validate input
  if (typeof username !== 'string' || typeof password !== 'string') {
    res.status(400).send('Invalid input: username and password must be strings');
    return;
  }
