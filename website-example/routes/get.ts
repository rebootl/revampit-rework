...
const locale = {
  en: {
    ...
    "byNumbersEnvDevices": "Devices saved from landfills annually through repair and refurbishment",
    "byNumbersEnvRefurb": "Of donated equipment successfully refurbished and given a second life",
    "byNumbersCommunityTitle": "Community Impact",
    "byNumbersCommunityTrained": "People trained in open source and sustainable technology annually",
    ...
    "ctaSectionText": "Whether you need a device repaired, want to learn about sustainable computing, or wish to support our cause, we welcome you to be part of our community. Together, we can make technology more sustainable and accessible for everyone.",
  ...
  de: {
    ...
    "byNumbersEnvDevices": "Jährlich durch Reparatur und Wiederaufbereitung vor der Deponie gerettete Geräte",
    "byNumbersEnvRefurb": "Anteil der gespendeten Geräte, die erfolgreich wiederaufbereitet und weitergegeben werden (%)",
    "byNumbersCommunityTitle": "Gemeinschaftliche Wirkung",
    "byNumbersCommunityTrained": "Jährlich in Open Source und nachhaltiger Technologie geschulte Personen",
    ...
    "ctaSectionText": "Ob du ein Gerät reparieren lassen möchtest, mehr über nachhaltiges Computing erfahren willst oder unsere Sache unterstützen möchtest – wir laden dich ein, Teil unserer Community zu werden. Gemeinsam können wir Technologie nachhaltiger und zugänglicher für alle machen.",
    "ctaSectionButton": "Mitmachen"
  }
};

function getNewsData(req: Request) {
  const language = (req as any).lang || 'en';
  try {
    const stmt = (req as any).db.prepare('SELECT * FROM entries WHERE type = ? AND draft = 0 AND language = ? ORDER BY created_at DESC LIMIT 1');
...
export default (req: Request, res: Response) => {
  const language = (req as any).lang || 'en';
  const newsData = getNewsData(req as Request);
...
<main class="pt-20 min-h-screen">
  ...
  <section class="py-20 px-4 max-w-6xl mx-auto">
    <div class="space-y-8">
      <h2 class="text-3xl font-bold">${locale[language].missionSectionTitle}</h2>
      <div class="space-y-6">
        <p class="text-lg">
  ...
  <section class="py-20 bg-gray-50">
    <div class="max-w-6xl mx-auto px-4">
      <h2 class="text-3xl font-bold mb-12 text-center">${locale[language].impactSectionTitle}</h2>
      <div class="grid md:grid-cols-3 gap-8">
        <div class="bg-white p-8 rounded-lg shadow-md">
          <h3 class="text-xl font-semibold mb-4">${locale[language].impactHardwareTitle}</h3>
          <div class="space-y-4 text-lg">
            ${locale[language].impactHardwareBullets.map((b: string) => `<p class="mb-4">${b}</p>`).join('')}
          </div>
        </div>
        <div class="bg-white p-8 rounded-lg shadow-md">
          <h3 class="text-xl font-semibold mb-4">${locale[language].impactOpenSourceTitle}</h3>
          <div class="space-y-4 text-lg">
            ${locale[language].impactOpenSourceBullets.map((b: string) => `<p class="mb-4">${b}</p>`).join('')}
          </div>
        </div>
        <div class="bg-white p-8 rounded-lg shadow-md">
          <h3 class="text-xl font-semibold mb-4">${locale[language].impactCommunityTitle}</h3>
          <div class="space-y-4 text-lg">
            ${locale[language].impactCommunityBullets.map((b: string) => `<p class="mb-4">${b}</p>`).join('')}
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="py-20 bg-white">
    <div class="max-w-6xl mx-auto px-4">
      <h2 class="text-3xl font-bold mb-12 text-center">${locale[language].byNumbersSectionTitle}</h2>
      <div class="grid md:grid-cols-2 gap-8">
        <div class="bg-green-50 p-8 rounded-lg shadow-md">
          <h3 class="text-2xl font-bold text-green-800 mb-2">${locale[language].byNumbersEnvTitle}</h3>
          <div class="space-y-6">
            <div>
              <p class="text-4xl font-bold text-green-700 mb-2">5+</p>
              <p class="text-gray-600">${locale[language].byNumbersEnvLifespan}</p>
            </div>
            <div>
              <p class="text-4xl font-bold text-green-700 mb-2">1000+</p>
              <p class="text-gray-600">${locale[language].byNumbersEnvDevices}</p>
            </div>
            <div>
              <p class="text-4xl font-bold text-green-700 mb-2">75%</p>
              <p class="text-gray-600">${locale[language].byNumbersEnvRefurb}</p>
            </div>
          </div>
        </div>
        <div class="bg-green-50 p-8 rounded-lg shadow-md">
          <h3 class="text-2xl font-bold text-green-800 mb-2">${locale[language].byNumbersCommunityTitle}</h3>
          <div class="space-y-6">
            <div>
              <p class="text-4xl font-bold text-green-700 mb-2">20+</p>
              <p class="text-gray-600">${locale[language].byNumbersCommunityTrained}</p>
            </div>
            <div>
              <p class="text-4xl font-bold text-green-700 mb-2">90%</p>
              <p class="text-gray-600">${locale[language].byNumbersCommunityInterns}</p>
            </div>
            <div>
              <p class="text-4xl font-bold text-green-700 mb-2">10+</p>
              <p class="text-gray-600">${locale[language].byNumbersCommunityReintegration}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  <section class="py-20 px-4 max-w-6xl mx-auto">
    <div class="space-y-8">
      <h2 class="text-3xl font-bold">${locale[language].storySectionTitle}</h2>
      <div class="space-y-6 text-lg">
        <p>
          ${locale[language].storySectionP1}
        </p>
        <p>
          ${locale[language].storySectionP2}
        </p>
        <p>
          ${locale[language].storySectionP3}
        </p>
      </div>
    </div>
  </section>
  ...
  <section class="py-20 bg-green-600 text-white">
    <div class="max-w-4xl mx-auto text-center px-4">
      <h2 class="text-3xl font-bold mb-6">${locale[language].ctaSectionTitle}</h2>
      <p class="text-xl mb-8">
        ${locale[language].ctaSectionText}
      </p>
      <a href="/get-involved" class="inline-block bg-white text-green-600 px-8 py-4 rounded-lg font-semibold hover:bg-gray-100 transition-colors">
        ${locale[language].ctaSectionButton}
      </a>
    </div>
  </section>
</main>
